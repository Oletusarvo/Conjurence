...
export const socketServer = io => {
  io.on('connection', socket => {
    ...
    socket.on('join_room', async roomName => joinRoomHandler(io, socket, roomName));
    ...
    socket.on('user:position_update', async payload =>
      userPositionUpdateHandler(io, socket, payload)
    );

    //Ran when the location of a user changes while hosting a mobile event.
    socket.on('event:position_update', async payload =>
      eventPositionUpdateHandler(io, socket, payload)
    );
  });
};
