...
export default async function ClientEventPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axios.get(`${networkConfig.api}/events/${event_id}`);
      if (res.status !== 200) {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axios.get(`${networkConfig.api}/attendance/${event_id}`);
      if (res.status !== 200) {
  ...
  return isPending || isAttendancePending ? (
    <Spinner />
  ) : (
